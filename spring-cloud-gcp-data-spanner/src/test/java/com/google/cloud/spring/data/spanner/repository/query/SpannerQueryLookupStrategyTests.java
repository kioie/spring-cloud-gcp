...
class SpannerQueryLookupStrategyTests {
  ...
  void getColumnsStringForSelectMultipleTest() {
    ...
    assertThat(childrenRowsQuery.getSql())
        .isEqualTo(
            "SELECT custom_col, deleted, id, id_2, other, ARRAY (SELECT AS STRUCT deleted, id,"
                + " id3, id_2 FROM child_test_table WHERE (child_test_table.id = custom_test_table.id"
