...
/**
 * Executes Cloud Spanner query statements using {@link
 * org.springframework.data.repository.query.parser.PartTree} parsed method definitions.
 *
 * @since 1.1
 */
public final class SpannerStatementQueryExecutor {
  ...
  public static <T> List<T> executeQuery(
    ...
    return spannerTemplate.query(
        ...
        buildStatementFromSqlWithArgs(
            sqlStringAndPlaceholders.getSql(),
            sqlStringAndPlaceholders.getPlaceholders(),
            null,
   ...
   * @param mappingContext a mapping context to convert between Cloud Spanner column names and
   *     underlying property names.
   * @param fetchInterleaved when {@code true} additional subqueries will be added to fetch
   ...
   */
  public static <T> String applySortingPagingQueryOptions(
      Class<T> entityClass,
  ...
  /**
   * Gets a {@link Statement} that returns the rows associated with a parent entity. This function
   * is intended to be used with parent-child interleaved tables, so that the retrieval of all child
   * rows having the parent's key values is efficient.
   *
   * @param parentKey the parent key whose children to get.
   * @param spannerPersistentProperty the property with interleaved list of child entries in the
   *     parent entity.
  ...
  /**
   * Builds a query that returns the rows associated with a key set. If the entity class has {@link
   * com.google.cloud.spring.data.spanner.core.mapping.Where} annotation it will be used to build
   * the query.
   *
  ...
  public static <T> Statement buildQuery(
      ...
      SpannerMappingContext mappingContext) {
    return buildQuery(
        keySet, persistentEntity, writeConverter, mappingContext, persistentEntity.getWhere());
  }

  /**
   * Builds a query that returns the rows associated with a key set with additional SQL-where. The
   * {@link com.google.cloud.spring.data.spanner.core.mapping.Where} of the {@code persistentEntity}
   * parameter is ignored, you should pass the SQL-where as a {@code whereClause} parameter.
   *
   * @param keySet the key set whose members to get.
   * @param persistentEntity the persistent entity of the table.
   * @param <T> the type of the persistent entity
   ...
   * Builds a query that returns the rows associated with a key set with additional SQL-where. The
   * {@link com.google.cloud.spring.data.spanner.core.mapping.Where} of the {@code persistentEntity}
   * parameter is ignored, you should pass the SQL-where as a {@code whereClause} parameter. The
   * secondary {@code index} will be used instead of the table name when the corresponding parameter
   * is not null.
   *
   * @param keySet the key set whose members to get.
   * @param persistentEntity the persistent entity of the table.
  ...
  public static <T> Statement buildQuery(
    ...
    int tagNum = 0;
    List<SpannerPersistentProperty> keyProperties =
        persistentEntity.getFlattenedPrimaryKeyProperties();
  ...
  private static <C, P> String getChildrenStructsQuery(
    ...
    String keylCause =
        parentKeyProperties.stream()
            .map(
                keyProp ->
                    tableName
                        + "."
                        + keyProp.getColumnName()
                        + " = "
                        + parentPersistentEntity.tableName()
                        + "."
                        + keyProp.getColumnName())
            .collect(Collectors.joining(AND));
    String condition = combineWithAnd(keylCause, whereClause);
    return "ARRAY (SELECT AS STRUCT "
        + getColumnsStringForSelect(childPersistentEntity, mappingContext, true)
        + " FROM "
        + tableName
        + WHERE
        + condition
  ...
  /**
   * Combines two SQL conditions with {@code AND} or returns only one when another is a null or
   * empty string. The method never return null but an empty string instead.
   *
   ...
   * @param paramStructConvertFunc a function to use to convert params to {@link Struct} objects if
   *     they cannot be directly mapped to Cloud Spanner supported param types. If null then this
   *     last-attempt conversion is skipped.
   * @param spannerCustomConverter a converter used to convert params that aren't Cloud Spanner
   *     native types. if {@code null} then this conversion is not attempted.
   * @param params the parameters to substitute the tags. The ordering must be the same as the tags.
   ...
   * @throws IllegalArgumentException if the number of tags does not match the number of params, or
   *     if a param of an unsupported type is given.
   */
  public static Statement buildStatementFromSqlWithArgs(
      String sql,
      List<String> tags,
    ...
    for (int i = 0; i < tags.size(); i++) {
      bindParameter(
          builder.bind(tags.get(i)),
          paramStructConvertFunc,
          spannerCustomConverter,
          params[i],
          queryMethodParams == null ? null : queryMethodParams.get(tags.get(i)));
    }
  ...
  private static void bindParameter(
      ValueBinder<Statement.Builder> bind,
      Function<Object, Struct> paramStructConvertFunc,
      SpannerCustomConverter spannerCustomConverter,
    ...
    if (ConversionUtils.isIterableNonByteArrayType(propType)) {
      if (!ConverterAwareMappingSpannerEntityWriter.attemptSetIterableValueOnBinder(
          (Iterable) originalParam,
          bind,
          spannerCustomConverter,
          (Class)
              ((ParameterizedType) paramMetadata.getParameterizedType())
    ...
    if (!ConverterAwareMappingSpannerEntityWriter.attemptBindSingleValue(
        originalParam, propType, bind, spannerCustomConverter)) {
      if (paramStructConvertFunc == null) {
        throw new IllegalArgumentException(
            "Param: " + originalParam + " is not a supported type: " + propType);
      }
      try {
        Object unused =
            ((BiFunction<ValueBinder, Object, Struct>)
                    ConverterAwareMappingSpannerEntityWriter.singleItemTypeValueBinderMethodMap.get(
                        Struct.class))
                .apply(bind, paramStructConvertFunc.apply(originalParam));
      } catch (SpannerDataException ex) {
        throw new IllegalArgumentException(
            "Param: " + originalParam + " is not a supported type: " + propType, ex);
      }
    }
  }
  ...
  public static String getColumnsStringForSelect(
      ...
      SpannerMappingContext mappingContext,
      boolean fetchInterleaved) {
    List<String> sortedColumns = new ArrayList<>(spannerPersistentEntity.columns());
  ...
  /**
   * Returns the value of the {@link Where} annotation of the Property or Persistent Entity. When
   * the {@link Where} is used on the Property it has higher priority then from Persistent Entity
   * and overrides it.
   *
  ...
  private static String getWhere(
    ...
    return spannerPersistentProperty.hasWhere()
        ? spannerPersistentProperty.getWhere()
        : childPersistentEntity.getWhere();
  }
  ...
  private static SqlStringAndPlaceholders buildPartTreeSqlString(
      PartTree tree,
      SpannerMappingContext spannerMappingContext,
      Class type,
    ...
    applySort(
        params.getSort().isSorted() ? params.getSort() : tree.getSort(),
        stringBuilder,
        persistentEntity);
  ...
  private static void buildFrom(
      SpannerPersistentEntity<?> persistentEntity, StringBuilder stringBuilder) {
    stringBuilder.append("FROM ").append(persistentEntity.tableName()).append(" ");
  }

  public static StringBuilder applySort(
      Sort sort, StringBuilder sql, SpannerPersistentEntity<?> persistentEntity) {
        ...
        .forEachRemaining(
            o -> {
              SpannerPersistentProperty property =
                  persistentEntity.getPersistentProperty(o.getProperty());
              String sortedPropertyName =
                  (property != null) ? property.getColumnName() : o.getProperty();
              String sortedProperty =
                  o.isIgnoreCase() ? LOWER_LHS + sortedPropertyName + ")" : sortedPropertyName;
              ...
              orPart -> {
                ...
                orPart.forEach(
                    part -> {
                      ...
                      SpannerPersistentProperty spannerPersistentProperty =
                          persistentEntity.getPersistentProperty(segment);

                      if (spannerPersistentProperty.isEmbedded()) {
                        throw new SpannerDataException(
                            "Embedded class properties are not currently supported in query method"
                                + " names: "
                                + segment);
                      }
                      ...
                      if (part.shouldIgnoreCase() == IgnoreCaseType.ALWAYS) {
                        ...
                        throw new SpannerDataException(
                            "Only ignore-case types ALWAYS and NEVER are supported, because the"
                                + " underlying table schema is not retrieved at query time to check"
                                + " that the column is the STRING or BYTES Cloud Spanner  type"
                                + " supported for ignoring case.");
                      }
                      ...
                      switch (part.getType()) {
                        ...
                        case CONTAINING:
                          andString =
                              " REGEXP_CONTAINS(" + andString + "," + insertedTag + ") =TRUE";
                          break;
                        case NOT_CONTAINING:
                          andString =
                              " REGEXP_CONTAINS(" + andString + "," + insertedTag + ") =FALSE";
                          break;
                        ...
                        default:
                          throw new UnsupportedOperationException(
                              "The statement type: " + part.getType() + " is not supported.");
                      }
